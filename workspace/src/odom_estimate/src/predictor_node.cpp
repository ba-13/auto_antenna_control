...
int main(int argc, char **argv) {
  ...
  ros::Subscriber sub_pose = nh.subscribe(
      "/virtual/pose", 1, &predictor::Predictor::set_position, &predictor);
  ros::Subscriber sub_vel = nh.subscribe(
      "/virtual/velocity", 1, &predictor::Predictor::set_velocity, &predictor);
  ros::Subscriber sub_acc = nh.subscribe(
      "/virtual/imu", 1, &predictor::Predictor::set_acceleration, &predictor);
  ros::Publisher pub_predicted_position =
      nh.advertise<geometry_msgs::PoseStamped>("/predicted/pose", 1);
